...
sql_query = "SELECT * FROM raw.vehicles"
...
try:
    ...
    numeric_columns = ['cost_in_credits', 'length', 'max_atmosphering_speed', 'crew', 'passengers', 'cargo_capacity']
    for column in numeric_columns:
        df[column] = pd.to_numeric(df[column], errors='coerce')

    # Define columns for each table
    table_columns = {
        'vehicles': ['id', 'name', 'model', 'manufacturer', 'cost_in_credits', 'length', 'max_atmosphering_speed', 'crew', 'passengers', 'cargo_capacity', 'consumables', 'vehicle_class', 'created', 'edited', 'url'],
        'vehicle_pilots': ['id', 'pilots', 'url'],
        'vehicle_films': ['id', 'films', 'url']
    }
